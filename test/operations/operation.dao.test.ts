...
describe.skip(' >>> Operation spatial querys <<< ', function () {
  before(async () => {
    await initAsync();
  ...
  it("get a operation when getOperationByVolume by volume intersection", async () => {
    ...
    const polygon: Polygon = {
      "type": "Polygon", 
      "coordinates": [ [ [ -56.16494178771972, -34.905184795559876 ], [ -56.16168022155762, -34.9083170799602 ], [ -56.15906238555908, -34.906135051768 ], [ -56.16198062896728, -34.90444569979538 ], [ -56.16494178771972, -34.905184795559876 ] ] ]
    };
  ...
  it("doesn't get a operation when getOperationByVolume because no operation intersection", async () => {
    ...
    const polygon: Polygon = {
      "type": "Polygon",
      "coordinates": [[[-56.1566162109375,-34.90567752237382],[-56.15899801254272,-34.9075076246545],[-56.153719425201416,-34.908704207937646],[-56.15395545959473,-34.91287451630941],[-56.154277324676514,-34.91468686277003],[-56.16358995437622,-34.91217068165463],[-56.16264581680298,-34.91475724434973],[-56.15311861038208,-34.9151795325607],[-56.15266799926758,-34.90581830092048],[-56.1566162109375,-34.90567752237382]]]
    };
    operation_volume.operation_geography = polygon
    operation_volume.beyond_visual_line_of_sight = true
    let opDao = new OperationDao()
